...
<body>
  ...
  <form id="marks-form" action="<?php echo htmlspecialchars(
      $_SERVER["PHP_SELF"]
  ); ?>" method="POST">
        ...
        <div class="row clearfix">
          <div class="">
            ...
            <label for="department">Department : </label>
              ...
              <span id="department-text"><?php echo getDepartmentName(
                  $department
              ); ?></span>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<a href='http://localhost/new1/excel_interface.php?table_name=" .
                urlencode($table_name) .
                "&department=" .
                urlencode($department) .
                "&redirect=true' target='_blank' class='paper-link'>$table_name</a>";
            ...
            $co_attainment_result = mysqli_query(
                $conn_co,
                $co_attainment_query
            );
            ...
            if (mysqli_num_rows($co_attainment_result) > 0) {
                ...
                while (
                    $co_attainment_row = mysqli_fetch_assoc(
                        $co_attainment_result
                    )
                ) {
        ...
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Year: " .
            $year .
            "</h3></div>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Department: " .
            $department .
            "</h3></div>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Division: " .
            $division .
            "</h3></div>";
        echo "</div>";

        echo "<div style='display:flex;justify-content:center;'>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Semester: " .
            $sem .
            "</h3></div>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Subject: " .
            $subject .
            "</h3></div>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Test Type: " .
            $testType .
            "</h3></div>";
        echo "</div>";

        echo "<div style='display:flex;justify-content:center;'>";
        echo "<div style='margin: 0 10px;'><h3 style='font-size: 1.17em; font-weight: bold;'>Academic Year: " .
            $academic_year .
            "</h3></div>";
        echo "</div>";
        ...
        $table_result = mysqli_query($conn, $table_exists_query);
        ...
        if ($table_result && mysqli_num_rows($table_result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $tooltip_data[$row["id"]] = $row["CO"];
            }
        } else {
            echo "The table $new_table_name does not exist.";
        }
        ...
        foreach ($combined_data as $co => $attainment_data) {
            ...
            if (!empty($attainment_data)) {
                $average =
                    array_sum($attainment_data) / count($attainment_data);
        ...
        echo "<td colspan='" .
            (count($test_types) + 1) .
            "'>FINAL DIRECT COURSE ATTAINMENT</td>";
        echo "<td colspan='2'>" .
            round($final_direct_course_attainment, 2) .
            "</td>"; // Set colspan to 2 for the value cell
        echo "</tr>";
        echo "</table>";
        echo '<br><a href="#" class="print-button" onclick="printTable()">Print Table</a>';
        echo "</div>";

        echo "<a href='add_co.php' target='_blank'>Add CO Values</a>";

        echo '<script>
        function printTable() {
            var printContents = document.getElementById("direct-attainment-table").outerHTML;
            var originalContents = document.body.innerHTML;
            var header = \'<img src="college_logo.jpeg" alt="College Logo" style="width:100%; max-height:150px;"><div style="display:flex;justify-content:center;"><div style="margin: 0 10px;"><h3>Year: ' .
            $year .
            '</h2></div><div style="margin: 0 10px;"><h3>Department: ' .
            $department .
            '</h2></div><div style="margin: 0 10px;"><h3>Division: ' .
            $division .
            '</h2></div></div><div style="display:flex;justify-content:center;"><div style="margin: 0 10px;"><h3>Semester: ' .
            $sem .
            '</h3></div><div style="margin: 0 10px;"><h3>Subject: ' .
            $subject .
            '</h3></div><div style="margin: 0 10px;"><h3>Test Type: ' .
            $testType .
            '</h3></div></div><div style="display:flex;justify-content:center;"><div style="margin: 0 10px;"><h3>Academic Year: ' .
            $academic_year .
            '</h3></div></div>\';
    ...
    echo '<div id="papers-count" style="background-color: ' .
        $color .
        '; width: 200px;">';
    echo "<p>Progress: " . $num_papers . " papers available</p>";
    echo "</div>";

    $CO_table_name = "Co_values_" . $subject;

    // Check if the table exists
    $table_exists_query = "SHOW TABLES LIKE '$CO_table_name'";
    $table_result = mysqli_query($conn, $table_exists_query);

    if ($table_result && mysqli_num_rows($table_result) > 0) {
        // Table exists, so fetch data from it
        $query = "SELECT * FROM $CO_table_name";
        $result = mysqli_query($conn, $query);
        ...
        // Display the data with styles
        echo "<table style='border: 1px solid black; width: 100%;'>";
        echo "<thead>";
        echo "<tr>";
        echo "<th style='border: 1px solid black; padding: 8px; background-color: #f2f2f2;'>ID</th>";
        echo "<th style='border: 1px solid black; padding: 8px; background-color: #f2f2f2;'>CO Value</th>";
        echo "</tr>";
        echo "</thead>";
        echo "<tbody>";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            echo "<td style='border: 1px solid black; padding: 8px;'>CO" .
                $row["id"] .
                "</td>";
        ...
        // Table doesn't exist
        echo "The CO values are not entred yet.";
    }
